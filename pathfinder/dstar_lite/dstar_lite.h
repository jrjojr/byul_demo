...
typedef struct s_dstar_lite {
    ...
    //      dstar_lite_find_path_incremental()
    gint max_range;
...
BYUL_API const path dstar_lite_get_proto_path(const dstar_lite dsl);

BYUL_API const path dstar_lite_get_real_path(const dstar_lite dsl);
 ...
 * @param al 알고리즘 컨텍스트
 */    
BYUL_API void dstar_lite_compute_shortest_path(dstar_lite dsl);
 ...
 *
 * @param dsl 알고리즘 컨텍스트
 * @return path 유효한 경로 객체, 실패 시 NULL
 */
BYUL_API path dstar_lite_reconstruct_path(const dstar_lite dsl);

// 1회용 경로 찾기 가장 단순한 형태 정적인 경로 찾기와 같다.
BYUL_API path dstar_lite_find(const dstar_lite dsl);
 ...
 * @param p 갱신할 경로
 */
BYUL_API void dstar_lite_update_vertex_by_path(dstar_lite dsl, path p);
