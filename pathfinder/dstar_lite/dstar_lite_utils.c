...
static char get_map_char(
    ...
    gboolean is_path    = path_coords && g_hash_table_contains(
        path_coords, tmp);
...
//     // hashset path_coords = NULL;
//     GHashTable* path_coords = g_hash_table_new_full(
//         (GHashFunc) coord_hash,
//         (GEqualFunc) coord_equal,
//         (GDestroyNotify) coord_free,
//         NULL);

//     if (p && path_get_success(p)) {
//         for (const GList* l = path_get_coords(p); l != NULL; l = l->next)
//             g_hash_table_add(path_coords, coord_copy((coord)l->data));
//     }
...
//         for (gint x = 0; x < dsl->m->width; x++) {
//             printf("%s", get_map_string(
//                 dsl->m, x, y, dsl->start, dsl->goal, path_coords, NULL));
//         }
//         putchar('\n');
//     }

//     // if (path_coords) hashset_free(path_coords);
//     if (path_coords) g_hash_table_destroy(path_coords);
// }
...
void dsl_print_ascii(const dstar_lite dsl, const path p) {
    ...
    if (p && path_get_success(p)) {
        for (const GList* l = path_get_coords(p); l != NULL; l = l->next) {
            ...
            g_hash_table_add(path_coords, coord_copy(c));
            min_x = MIN(min_x, c->x);
            max_x = MAX(max_x, c->x);
            min_y = MIN(min_y, c->y);
            max_y = MAX(max_y, c->y);
    ...
    printf("[MAP %d,%d to %d,%d with PATH]\n", min_x, min_y, max_x, max_y);
    ...
    g_hash_table_destroy(path_coords);
}
...
void dsl_print_ascii_uv(const dstar_lite dsl, const path p) {
    ...
    if (p && path_get_success(p)) {
        for (const GList* l = path_get_coords(p); l != NULL; l = l->next) {
            ...
            g_hash_table_add(path_coords, c_copy);
    ...
    for (gint y = min_y; y <= max_y; y++) {
        for (gint x = min_x; x <= max_x; x++) {
            ...
            if (coord_equal(c, dsl->start)) {
                ...
                printf("  E");
            } else if (g_hash_table_contains(path_coords, c)) {
                printf("  *");
