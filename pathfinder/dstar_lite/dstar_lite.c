...
void dstar_lite_free(dstar_lite dsl) {
    ...
    if (dsl->proto_path) path_free(dsl->proto_path);
    if (dsl->real_path) path_free(dsl->real_path);
    g_free(dsl);
}
...
void dstar_lite_reset(dstar_lite dsl) {
    ...
    if (dsl->proto_path) {
        path_free(dsl->proto_path);
        dsl->proto_path = NULL;
    }

    if (dsl->real_path) {
        path_free(dsl->real_path);
        dsl->real_path = NULL;
    }    
...
void dstar_lite_update_vertex_by_path(const dstar_lite dsl, const path p) {
    if (!dsl || !p) return;
...
void dstar_lite_compute_shortest_path(dstar_lite dsl) {
    ...
    do {
        loop++;
        // g_print("dstar_lite_compute_shortest_path "
        //     "내부에서 루프 %d 시작.\n", loop);
...
path dstar_lite_reconstruct_path(const dstar_lite dsl) {
    ...
    while (!coord_equal(current, dsl->goal) && 
        ...
        loop++;
        // g_print("dstar_lite_reconstruct_path "
        //     "내부에서 루프 %d 시작.\n", loop);
...
path dstar_lite_find(const dstar_lite dsl) {
    ...
    dstar_lite_compute_shortest_path(dsl);
    return dstar_lite_reconstruct_path(dsl);
}
...
void dstar_lite_find_proto(const dstar_lite dsl) {
    if (!dsl) return;
    // dstar_lite_reset(dsl);
    dsl->proto_path = dstar_lite_find(dsl);
}
 
void dstar_lite_find_loop(const dstar_lite dsl) {
    if (!dsl) return;

    // if (!dsl->proto_path) dstar_lite_find_proto(dsl);

    coord s_last = coord_copy(dsl->start);
    coord start = coord_copy(dsl->start);

    dsl->real_path = path_new();
    path_add_coord(dsl->real_path, start);
    ...
    while (!coord_equal(start, dsl->goal) && 
        ...
        if (float_equal(rhs_start, FLT_MAX)) {
            path_set_success(dsl->real_path, FALSE);
        ...
        path_add_coord(dsl->real_path, next);
    ...
    path_set_success(dsl->real_path, TRUE);
    dsl->real_loop_retry_count = loop;
    if (s_last) coord_free(s_last);    
        dsl->force_quit = FALSE;
    return;    
